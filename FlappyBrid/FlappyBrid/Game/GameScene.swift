...
class GameScene: SKScene {
    ...
    lazy var bird = SKSpriteNode(texture: birdTexture1).then { bird in
        bird.setScale(2.0)
        bird.position = CGPoint(x: self.width * 0.35, y:self.height * 0.6)
    ...
    override func didMove(to view: SKView) {
        ...
        for i in 0 ..< 2 + Int(self.width/groundWidth) {
        ...
        for i in 0 ..< 2 + Int(self.width/skyWidth) {
    ...
    private func spawnPipes() {
        ...
        let distanceToMove = self.width + 2.0 * pipeTextureUp.width
...
extension GameScene: SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if ( contact.bodyA.categoryBitMask & PhysicsCatagory.score ) == PhysicsCatagory.score || ( contact.bodyB.categoryBitMask & PhysicsCatagory.score ) == PhysicsCatagory.score {
            ...
            let repeatSequence = SKAction.repeat(SKAction.sequence([red, wait, sky, wait]), count:3)
            let finished = SKAction.run {
                self.isUserInteractionEnabled = true
            }
            run(SKAction.sequence([repeatSequence, SKAction.wait(forDuration: 0.5), finished]), withKey: "flash_background_color")
        }
    }
}
